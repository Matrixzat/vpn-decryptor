...
class NetModDecoder:
    ...
    def decode_bytes(cls, file_bytes: bytes) -> str:
        ...
        try:
            plaintext = AES.new(cls.KEY, AES.MODE_ECB).decrypt(ciphertext)
        except Exception as exc:
            raise NetModDecodeError("AES-ECB decryption failed.") from exc
        ...
        try:
            ...
            raise NetModDecodeError(
                "Decrypted NetMod data is not valid UTF-8."
            ) from exc
